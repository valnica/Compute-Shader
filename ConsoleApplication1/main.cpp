...
int main()
{
	...
	if (FAILED(hr))
	{
		std::cout << "ƒfƒoƒCƒX¶¬‚Ì¸”s" << std::endl;
		getchar();
		return 0;
	}
	...
	if (FAILED(hr))
	{
		std::cout << "shader compile‚Ì¸”s" << std::endl;
		device->Release();
		getchar();
		return 0;
	}
	...
	if (FAILED(hr))
	{
		std::cout << "Compute Shader¶¬‚Ì¸”s" << std::endl;
		device->Release();
		context->Release();
		getchar();
		return 0;
	...
	if (FAILED(hr))
	{
		std::cout << "Resource View¶¬‚Ì¸”s" << std::endl;
		device->Release();
		context->Release();
		computeShader->Release();
		buffer->Release();
	...
	if (FAILED(hr))
	{
		std::cout << "Unordered Access View¶¬‚Ì¸”s" << std::endl;
		device->Release();
		context->Release();
		computeShader->Release();
		buffer->Release();
